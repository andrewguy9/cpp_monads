...
Maybe<B> monadBind(const Maybe<A> & m, std::function<Maybe<B>(A)> f)
{
  if (isJust(m)) {
    return f(fromJust(m));
  } else {
...
template<class A>
Maybe<A> monadPlus(const Maybe<A> & a, const Maybe<A> & b) {
  if (isJust(a)) {
    return a;
  } else {
    return b;
...
Maybe<R> liftM2(std::function<R (A1,A2)> f, const Maybe<A1> & mx, const Maybe<A2> & my) {
...
template<class A>
Maybe<A> join(const Maybe< Maybe<A> > & mma) {
  if (isJust(mma)) {
